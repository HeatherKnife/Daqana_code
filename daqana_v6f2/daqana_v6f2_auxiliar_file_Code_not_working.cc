...
*/
  ...
  double min = get_min( smooth[ch], pos_min, 0, 2*pre_tr_smp); //The program returns in some way the value of x, which is pos_min through a pointer or a direction of memory
  double bl_chi2 = fit_line( raw[ch], bl_k, bl_m, 0, pre_tr_smp/2 );
  double bl_pre  = baseline( smooth[ch], 0, pre_tr_smp/2 );
  double bl_preRaw  = baseline( raw[ch], 0, pre_tr_smp/2 );
  double pulse_h_end = baseline(smooth[ch], smp_per_rec - pre_tr_smp/2, smp_per_rec-3);
  double start_bl_post = pos_min - 5;
  double bl_post;
  if (start_bl_post < 0)
  	bl_post = baseline(smooth[ch], 0, pos_min + 5);
  else
  	bl_post = baseline(smooth[ch], start_bl_post, pos_min + 5);
  double pos; //pos variable definition
  double val;
  double time;
//  double time_cfd; 
...
// For MCP, use Raw signal for timing;
  ...
  time = left_threshold( raw[ch], pos, bl_pre + 0.3 * (val - bl_pre) );
...
double ph = fabs(bl_pre - pulse_h_end);  //Here PH is builded
//double ph=val;

std::cout << "start_bl_post " << start_bl_post << " bl_pre " << bl_pre << " bl_post " << bl_post << " ph " << ph << " min " << min << " pos_min " << pos_min << " pulse_h_end " << pulse_h_end << std::endl;
