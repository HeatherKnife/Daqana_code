...
*/
  ...
  double bl_pre  = baseline( smooth[ch], 0, pre_tr_smp/2 );  //First average for the beggining of the somooth pulse
  double bl_preRaw  = baseline( raw[ch], 0, pre_tr_smp/2 );  //First average for the beggining of the raw pulse
  double start_bl_post = pos_min - 5.0;
  double bl_post;
  if (start_bl_post < 0)
    bl_post = baseline(smooth[ch], 0, pos_min + 5.0);
  else
    bl_post = baseline(smooth[ch], start_bl_post, (pos_min + 5) -(start_bl_post));
  ...
  //double sup_limit_PH = smp_per_rec-10;
...
//double ph=val;
 ...
 std::cout << "start_bl_post " << start_bl_post << " bl_post " << bl_post << " bl_pre " << bl_pre << " ph " << ph << " min " << min << " pos_min " << pos_min << std::endl;
